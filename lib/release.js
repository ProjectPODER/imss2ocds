...
function tenderObject(items) {
    ...
    const document = {
        ...
        procurementMethodDetails: items[0].procedimiento_unificado
    };
    ...
    let tenderItems = [];
    items.map( item => {
        if(isValidItem(item)) tenderItems.push(getTenderItem(item));
        total_amount += parseFloat(item.monto);
    } );
    Object.assign(document, { value: { amount: total_amount } });
    if(tenderItems.length > 0)
        Object.assign(document, { items: tenderItems });
...
function awardObject(items) {
    ...
    items.map( item => {
        ...
        if(isValidItem(item)) awardItems.push(getTenderItem(item));
        total_amount += parseFloat(item.monto);
    } )
    ...
    if(awardItems.length > 0)
        Object.assign(awardObject, { items: awardItems });
...
function getContractItem(item) {
    ...
    if(item.hasOwnProperty('monto') && item.monto != null) Object.assign(itemObj, { valueMxIMSS: parseFloat(item.monto) });
    ...
    if(item.hasOwnProperty('precio_unitario') && item.precio_unitario != null) Object.assign(itemObj.unit, { value: { amount: parseFloat(item.precio_unitario), quantityLostMxIMSS: 0, currency: 'MXN' } });
    ...
    if(item.hasOwnProperty('monto_diferencia') && item.monto_diferencia != null) {
        Object.assign(itemObj.unit.value, { amountOverpriceMxIMSS: item.monto_diferencia });
        if(item.monto_diferencia > 0) {
            let quantityLost = parseFloat( item.monto_diferencia / item.precio_ponderado );
            Object.assign(itemObj.unit.value, { quantityLostMxIMSS: quantityLost });
        }
    }
...
function isValidItem(item) {
    if(item.hasOwnProperty('clave_producto')) return true;
    return false;
}
...
function contractObject(items) {
    ...
    if(contractObj.period.startDate.match('1970')) debug = true;
    if(contractObj.period.startDate.match('1970')) contractObj.period.startDate = dateToISOString(items[0].year + '-01-01');
    if(contractObj.period.endDate.match('1970')) contractObj.period.endDate = dateToISOString(items[0].year + '-01-01');
    if(items[0].hasOwnProperty('iva') && items[0].iva != null) Object.assign(contractObj.value, { taxMxIMSS: items[0].iva })
