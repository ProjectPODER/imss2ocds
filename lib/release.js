...
function getContractItem(item) {
    ...
    if(item.hasOwnProperty('precio_unitario') && item.precio_unitario != null) Object.assign(itemObj.unit, { value: { amount: parseFloat(item.precio_unitario) } });
    if(item.hasOwnProperty('mongo')) Object.assign(itemObj.unit.value, { currency: 'MXN' });
    ...
    if(item.hasOwnProperty('monto_diferencia') && item.monto_diferencia != null) {
        ...
        if(item.monto_diferencia > 0) {
            ...
            Object.assign(itemObj.unit.value, { quantityLostMxIMSS: 0 });
        }
    }
